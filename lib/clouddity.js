...
module.exports.run = function(grunt, options, done) {
  ...
  var runIterator = function(image, next) {
    ...
    grunt.log.ok("Started creating and running tha container from "
    ...
    // If the newtwork mode is "bridge", adds all the hosts, and the current
    // node address as Hostname and "dockerhost"
    if (createOptions.NetworkMode.toLowerCase() !== "host") {
