...
/**
 * List all the flaovors available
 * 
 * @param {Object}
 *          grunt The Grunt instance
 * @param {Object}
  ...
  pkgcloud.compute.createClient(options.pkgcloud.client).getFlavors(
      function (err, flavors) {
        ...
        _.forEach(flavors, function(flavor) {
          console.log([ flavor.id, flavor.name, flavor.ram,flavor.disk, flavor.vcpus, flavor.swap ].join(", "));
        });
        done();
   });
};
...
module.exports.copytohost = function(grunt, options, done) {
  ...
  var copyIterator = function(node, nextNode) {
    if (!utils.isNodeToBeProcessed(grunt, node.node.type,
        node.node.id)) {
      return nextNode();
    }
    ...
    async.eachSeries(node.copytohost,
        function(volume, nextVolume) {
          ...
          exec("scp " + recursiveOption + " -o StrictHostKeyChecking=no -i "
              + node.ssh.privateKeyFile + " -P "
              + node.ssh.port + " " + volume.from + " "
...
module.exports.exec = function(grunt, options, done) {
  ...
  if (!grunt.option("command")) {
    utils.dealWithError({message: "Command option should not be empty"}, function(err) {
    });
    return done();
  }
  ...
  var execIterator = function(container, next) {
    ...
    grunt.log.ok("Started executing command on container " + container.container.Id
        + "  on node " + container.node.node.address);
    (new Docker(container.node.docker)).getContainer(container.container.Id)
        .exec({
          AttachStdout: true,
  ...
  utils.iterateOverClusterContainers(grunt, options, execIterator, function(
      err) {
    utils.dealWithError(err, function(err) {
    });
...
module.exports.export = function(grunt, options, done) {
  ...
  var exportIterator = function(container, next) {
    ...
    var fileName= container.container.Image + "-" + container.node.node.address + "-" + container.container.Id + ".tar";
  ...
  utils.iterateOverClusterContainers(grunt, options, exportIterator, function(
      err) {
    utils.dealWithError(err, function(err) {
    });
