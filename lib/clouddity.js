...
module.exports.pull = function (grunt, options, done) {
  ...
  utils.iterateOverClusterImages(grunt, options, function (image, next) {
    ...
    (new Docker(utils.addDockerHeaders(image.node.docker, options))).pull(
      image.repo, image, function (err, stream) {
      if (err) {
        return next(err);
      }
...
module.exports.run = function (grunt, options, done) {
  ...
  var runIterator = function (image, next) {
    ...
    var streamo = (new Docker(utils.addDockerHeaders(image.node.docker, options))).run(image.repo,
    ...
    streamo.on("container", function (container) {
      ...
      (new Docker(utils.addDockerHeaders(image.node.docker, options))).getContainer(container.id).start(
        {},
        function (err, data) {
...
module.exports.start = function (grunt, options, done) {
  ...
  var startIterator = function (container, next) {
    ...
    (new Docker(utils.addDockerHeaders(container.node.docker, options))).getContainer(container.container.Id)
          ...
          function () {
            if (body.indexOf(testcase.shouldStartWith) === 0) {
              ...
              if (body.indexOf(testcase.shouldContain) >= 0) {
                ...
                if (res.statusCode === testcase.shouldReturnStatus) {
                  grunt.log.ok("Test " + testcase.name
                    + " successfully completed");
                } else {
                  grunt.log.error(`Test ${testcase.name } in error`);
                  grunt.log.error(JSON.stringify(reqOptions));
                  grunt.log.error(`Status: ${res.statusCode} Body: ${body.substr(1, 300)} ...`);
                }
              }
            }
